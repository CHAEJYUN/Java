...
public class 전화기 {
	...
	public int price; //멤버변수는 자동 초기화(0) int price = 0;
	public String shape; //기본형이 아닌 경우(참조형) 모두 null로 초기화
	
	//눈에 보이진 않지만 기본 생성자가 만들어져 있음(묵시적으로)
	//=> 객체 생성할 때마다 이 메서드를 자동호출하여
	//   전화기 객체가 만들어졌음(출력하고 싶을 때?)

	//동작 : 멤버메서드(함수와 동일), 동적인 기능을 수행
	//       인터넷하다, 카톡하다
	public void 인터넷하다() {
		//기능을 처리하는 방법 나열
		System.out.println("와이파이를 연결하다");
		System.out.println("데이터를 연결하다");
	}
